...
public final class PreEncodedCodeCache implements AutoCloseable {
    ...
    private final ThreadLocal<ByteBuffer> warmByte = ThreadLocal.withInitial(() -> ByteBuffer.allocateDirect(1));

    private PreEncodedCodeCache(MappedByteBuffer[] chunks, int codeSize, int codesPerChunk,
                                FileChannel channel, long baseOffset) {
        this.chunks = chunks;
     ...
     * {@code offset + sectionBytes(count, codeSize)} bytes long.
     *
    ...
    /** As {@link #map(FileChannel, long, int, int)} with an explicit chunk target, so tests can exercise the multi-chunk path. */
    static PreEncodedCodeCache map(FileChannel fc, long offset, int count, int codeSize, int maxChunkBytes)
        ...
        return new PreEncodedCodeCache(chunks, codeSize, codesPerChunk, fc, offset);
    }

    /** Bytes of the mapped region for {@code count} codes. */
    ...
    /** Reads the code at {@code ordinal} as little-endian floats straight from the mapping. */
    ...
    /**
     * Faults in every page the row at {@code ordinal} spans using small positional reads rather
     * than touches on the mapping. A fault on a mapping drags the kernel's mmap readahead window
     * in with it (measured at 60-63 KB on the compaction scratch), which is nearly all waste for
     * a row of a few KB whose neighbours in ordinal order are not wanted; a small positional read
     * of the same pages is served as a standalone random read and pulls only what it asks for.
     * Pages land in the shared page cache either way, so the mapping read that follows hits.
     *
     * <p>The same technique as the native reader's {@code ReaderSupplier.prefetch}. Kept separate
     * because that one hangs off a source-file reader, this cache maps the output file, and the
     * default provider's {@code prefetch} is a no-op, which would silently disable warming.
     */
    public void warm(int ordinal) {
        long base = baseOffset + (long) ordinal * codeSize;
        ByteBuffer b = warmByte.get();
        try {
            for (int off = 0; off < codeSize; off += 4096) {
                b.clear();
                channel.read(b, base + off);
            }
            b.clear();
            channel.read(b, base + codeSize - 1);
        } catch (IOException e) {
            // warming is best effort: the mapping read that follows will fault the row in anyway
        }
    }
