 ...
 * <ul>
 *   <li><strong>{@link io.github.jbellis.jvector.status.StatusTracker}</strong> - The main monitoring wrapper
 *       that provides background polling and sink notification for any task object</li>
 *   <li><strong>{@link io.github.jbellis.jvector.status.TrackerScope}</strong> - Configuration and
 *       lifecycle management for creating tracker instances with hierarchical organization</li>
 *   <li><strong>{@link io.github.jbellis.jvector.status.StatusSink}</strong> - Processing interface for
 *       task lifecycle events, enabling multiple concurrent monitoring strategies</li>
 *   <li><strong>{@link io.github.jbellis.jvector.status.StatusMonitor}</strong> - Background polling engine
 *       that continuously monitors task status and detects changes</li>
 * </ul>
 *
 * <h3>Supporting Types</h3>
 * <ul>
 *   <li><strong>{@link io.github.jbellis.jvector.status.StatusUpdate}</strong> - Immutable status
 ...
 *   <li><strong>{@link io.github.jbellis.jvector.status.sinks.MetricsStatusSink}</strong> - Performance
 *       metrics collection with detailed statistics and reporting</li>
 *   <li><strong>{@link io.github.jbellis.jvector.status.sinks.NoopStatusSink}</strong> - Zero-overhead
 ...
 * }</pre>
 *
 * <h3>Scope-Based Configuration</h3>
 * <pre>{@code
 * // Hierarchical configuration with multiple sinks
 * TrackerScope scope = new TrackerScope("data-processing");
 * scope.addSink(new ConsoleTaskSink());
 * scope.addSink(new MetricsTaskSink());
 *
 * try (Tracker<DataProcessor> tracker = scope.track(processor)) {
 ...
 * try (TrackerScope scope = new TrackerScope("batch-job")) {
 *     List<Tracker<MyTask>> trackers = createTasks(scope);
 *
 ...
 * @see io.github.jbellis.jvector.status.TrackerScope
 * @see io.github.jbellis.jvector.status.StatusSink
 */
