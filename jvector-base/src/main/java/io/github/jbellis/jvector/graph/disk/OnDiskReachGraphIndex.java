 ...
 */
public class OnDiskReachGraphIndex implements GraphIndex, AutoCloseable, Accountable
{
    private static final Logger logger = LoggerFactory.getLogger(OnDiskReachGraphIndex.class);
     ...
     */
    public static OnDiskReachGraphIndex load(ReaderSupplier readerSupplier, long offset) {
        try (var reader = readerSupplier.get()) {
            ...
            logger.debug("Position after reading header={}",
                    reader.getPosition());
            return new OnDiskReachGraphIndex(readerSupplier, header, reader.getPosition());
        } catch (Exception e) {
     ...
     */
    public static OnDiskReachGraphIndex load(ReaderSupplier readerSupplier) {
    ...
    public class View implements FeatureSource, ScoringView, RandomAccessVectorValues {
        ...
        public int size() {
            ...
            return OnDiskReachGraphIndex.this.size(0);
        }
    ...
    @VisibleForTesting
    static boolean areHeadersEqual(OnDiskReachGraphIndex g1, OnDiskReachGraphIndex g2) {
