...
public final class MultiSearchResult {
    ...
    public MultiSearchResult(NodeScore[] nodes, int visitedCount, int expandedCount, int expandedCountL0, int rerankedCount, float worstApproximateScoreInTopK) {
        this.nodes = nodes;
    ...
    public static final class NodeScore implements Comparable<NodeScore> {
        ...
        public NodeScore(int index, int node, float score) {
            this.index = index;
            this.node = node;
            this.score = score;
        }
        ...
        public int compareTo(NodeScore o) {
            ...
            scoreCompare = scoreCompare != 0 ? scoreCompare : Integer.compare(node, o.node);
            // If scores are equal, break ties using index id (ascending order)
            scoreCompare = scoreCompare != 0 ? scoreCompare : Integer.compare(index, o.index);
            return scoreCompare;
        }
        ...
        public boolean equals(Object o) {
            ...
            return index == nodeScore.index && node == nodeScore.node && Float.compare(score, nodeScore.score) == 0;
        }

        @Override
        public int hashCode() {
            return Objects.hash(index, node, score);
        }
    }
