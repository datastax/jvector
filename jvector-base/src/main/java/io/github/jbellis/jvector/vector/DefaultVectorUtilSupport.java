...
final class DefaultVectorUtilSupport implements VectorUtilSupport {
  ...
  /*
   * In this implementation, compressedNeighbors is a matrix stored in row-major format where each row is a PQ code:
   * | pqCode[0]      |
   * | pqCode[1]      |
   * | pqCode[length] |
   */
  ...
  @Override
  public void bulkShuffleQuantizedSimilarity(ByteSequence<?> shuffles, int codebookCount, ByteSequence<?> quantizedPartials, float delta, float minDistance, VectorSimilarityFunction vsf, VectorFloat<?> results) {
    switch (vsf) {
      case EUCLIDEAN:
  ...
  private static void bulkShuffleQuantizedSimilarityEuclidean(ByteSequence<?> shuffles, int codebookCount,
                                                              ByteSequence<?> quantizedPartials, float delta,
                                                              float minDistance, VectorFloat<?> results) {
    for (int j = 0; j < results.length(); j++) {
      int val = 0;
      for (int i = 0; i < codebookCount; i++) {
        var shuffle = computeSingleShuffle(i, j, shuffles, codebookCount);
