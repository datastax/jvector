 ...
 *     scope.addSink(NoopTaskSink.getInstance());
 *
 *     try (Tracker<TestTask> tracker = scope.track(testTask)) {
 *         testTask.execute();
 ...
 * }</pre>
 *
 * <h3>Default Sink in Scope Configuration</h3>
 * <pre>{@code
 * public TrackerScope createScope(String name, boolean enableMonitoring) {
 *     TrackerScope scope = new TrackerScope(name);
 *     if (enableMonitoring) {
