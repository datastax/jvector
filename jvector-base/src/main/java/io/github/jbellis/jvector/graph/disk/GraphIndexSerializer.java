...
public interface GraphIndexSerializer {
    ...
    void writeSparseLevels(ImmutableGraphIndex graph, OrdinalMapper ordinalMapper, IndexWriter out, Map<FeatureId, IntFunction<Feature.State>> featureStateSuppliers) throws IOException;

    void writeHeader(ImmutableGraphIndex graph, OrdinalMapper ordinalMapper, Map<FeatureId,Feature> featureMap, int dimension, long startOffset, IndexWriter out, long headerSize) throws IOException;

    long featureOffsetForOrdinal(ImmutableGraphIndex graph, List<Feature> inlineFeatures, long startOffset, int ordinal, long headerSize);

    void writeFooter(ImmutableGraphIndex graph, OrdinalMapper ordinalMapper, long headerOffset, int dimension, IndexWriter out, Map<FeatureId,Feature> featureMap, long headerSize) throws IOException;

    void writeSeparatedFeatures(Map<FeatureId, IntFunction<Feature.State>> featureStateSuppliers, Map<FeatureId,Feature> featureMap, IndexWriter out, OrdinalMapper ordinalMapper) throws IOException;
    }