...
/**
 * Write a graph index to disk, for later loading as an OnDiskReachGraphIndex.
 * <p>
 ...
 * [Separated features]
 */
public class OnDiskReachGraphIndexWriter implements Closeable {
    ...
    private OnDiskReachGraphIndexWriter(RandomAccessWriter out,
                                        int version,
                                        long startOffset,
                                        GraphIndex graph,
    ...
    /**
     * Caller should synchronize on this OnDiskReachGraphIndexWriter instance if mixing usage of the
    ...
    public synchronized void write(Map<FeatureId, IntFunction<Feature.State>> featureStateSuppliers) throws IOException
    {
        ...
        for (int newOrdinal = 0; newOrdinal <= ordinalMapper.maxOrdinal(); newOrdinal++) {
            ...
            for (int n = 0; n < neighbors.size(); n++) {
                ...
                reachableGraph.get(newNeighborOrdinal).add(newOrdinal);
            }
        }
    ...
    /**
     * Writes the index header, including the graph size, so that OnDiskReachGraphIndex can open it.
     * The output IS flushed.
     * <p>
     * Public so that you can write the index size (and thus usefully open an OnDiskReachGraphIndex against the index)
    ...
    /**
     * Builder for OnDiskReachGraphIndexWriter, with optional features.
     */
    public static class Builder {
        ...
        public Builder(GraphIndex graphIndex, RandomAccessWriter out) {
            ...
            this.version = OnDiskReachGraphIndex.CURRENT_VERSION;
        }

        public Builder withVersion(int version) {
            if (version > OnDiskReachGraphIndex.CURRENT_VERSION) {
        ...
        public OnDiskReachGraphIndexWriter build() throws IOException {
            ...
            return new OnDiskReachGraphIndexWriter(out, version, startOffset, graphIndex, ordinalMapper, dimension, features);
        }
