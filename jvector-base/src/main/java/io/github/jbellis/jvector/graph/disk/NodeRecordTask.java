    ...
    public List<Result> call() throws Exception {
        ...
        for (int newOrdinal = startOrdinal; newOrdinal < endOrdinal; newOrdinal++) {
            ...
            if (originalOrdinal == OrdinalMapper.OMITTED) {
                ...
                // Write inline features - check each supplier individually
                // This follows the same logic as OnDiskGraphIndexWriter
                for (var feature : inlineFeatures) {
                    var supplier = featureStateSuppliers.get(feature.id());
                    if (supplier == null) {
                        // Feature was written separately via writeInline() - skip past it
                        // Advance the buffer position without overwriting the existing data
                        writer.getBuffer().position(writer.getBuffer().position() + feature.featureSize());
                    } else {
                        // Write the feature inline
