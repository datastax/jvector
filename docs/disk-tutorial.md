...
DataSet dataset = DataSetLoader.loadDataSet("ada002-100k");
```
...
```sh
mvn compile exec:exec@example -Dexample=disk -pl jvector-examples -am
```
The full example also illustrates the impact that adjusting `rerankK` has on recall.

Next steps:
- Building larger than memory indexes with Product Quantization
