...
public class AshDriver4Bit implements AshDriver {
    ...
    private static final long PIECE_MASK = 0b1111;
    private static final long WORD_MASK = ~(~0L << BITS_PER_WORD);

    private final int dimension;

    public AshDriver4Bit(int dimension) {
        ...
        for (int stack = 0; stack < stacks; stack++) {
            ...
            for (int lvl = 0; lvl < dimsPerWord; lvl++) {
                ...
                for (int wo = 0; wo < STACK_WIDTH; wo++) {
                    ...
                    long word = binaryVector[w] & WORD_MASK;
                    word |= ((long) qval << (lvl * BIT_DEPTH));
                    binaryVector[w] = (int) word;
                }
            }
        }
    }
        ...
        for (int stack = 0; stack < stacks; stack++) {
            ...
            for (int lvl = 0; lvl < dimsPerWord; lvl++) {
                ...
                for (int wo = 0; wo < STACK_WIDTH; wo++) {
                    ...
                    int w = stack * STACK_WIDTH + wo;
                    long word = binaryVector[w] & WORD_MASK;
                    int piece = (int) ((word >>> (lvl * BIT_DEPTH)) & PIECE_MASK);
    ...
    public float asymmetricScorePackedInts(PackedVectors packed, int pvOffset, float[] query, int qOffset) {
        ...
        var dotPieces4 = FloatVector.zero(FLOAT_SPECIES);
        var dotPieces5 = FloatVector.zero(FLOAT_SPECIES);
        var dotPieces6 = FloatVector.zero(FLOAT_SPECIES);
        var dotPieces7 = FloatVector.zero(FLOAT_SPECIES);
        var extraDot = 0.f;

        int i = 0;
        int stack = 0;
        for (; stack < stacks; stack++) {
            if (i + FLOATS_PER_VEC * 8 > dimension) {
                // Handle residual
                break;
            }
            ...
            var shifted4 = fullPiece.lanewise(VectorOperators.LSHR, BIT_DEPTH * 4);
            var shifted5 = fullPiece.lanewise(VectorOperators.LSHR, BIT_DEPTH * 5);
            var shifted6 = fullPiece.lanewise(VectorOperators.LSHR, BIT_DEPTH * 6);
            var shifted7 = fullPiece.lanewise(VectorOperators.LSHR, BIT_DEPTH * 7);
            ...
            var masked4 = shifted4.lanewise(VectorOperators.AND, PIECE_MASK);
            var masked5 = shifted5.lanewise(VectorOperators.AND, PIECE_MASK);
            var masked6 = shifted6.lanewise(VectorOperators.AND, PIECE_MASK);
            var masked7 = shifted7.lanewise(VectorOperators.AND, PIECE_MASK);
            ...
            var piece4 = masked4.convert(VectorOperators.I2F, 0);
            var piece5 = masked5.convert(VectorOperators.I2F, 0);
            var piece6 = masked6.convert(VectorOperators.I2F, 0);
            var piece7 = masked7.convert(VectorOperators.I2F, 0);
            ...
            var queryPiece4 = FloatVector.fromArray(FLOAT_SPECIES, query, qOffset + i + FLOATS_PER_VEC * 4);
            var queryPiece5 = FloatVector.fromArray(FLOAT_SPECIES, query, qOffset + i + FLOATS_PER_VEC * 5);
            var queryPiece6 = FloatVector.fromArray(FLOAT_SPECIES, query, qOffset + i + FLOATS_PER_VEC * 6);
            var queryPiece7 = FloatVector.fromArray(FLOAT_SPECIES, query, qOffset + i + FLOATS_PER_VEC * 7);
            i += FLOATS_PER_VEC * 8;
            ...
            dotPieces4 = queryPiece4.fma(piece4, dotPieces4);
            dotPieces5 = queryPiece5.fma(piece5, dotPieces5);
            dotPieces6 = queryPiece6.fma(piece6, dotPieces6);
            dotPieces7 = queryPiece7.fma(piece7, dotPieces7);
        }

        var dotPiecesL = dotPieces0.add(dotPieces1).add(dotPieces2.add(dotPieces3));
        var dotPiecesH = dotPieces4.add(dotPieces5).add(dotPieces6.add(dotPieces7));
        var dotPieces = dotPiecesL.add(dotPiecesH);
    ...
    public float symmetricScorePackedInts(PackedVectors a, int aOffset, PackedVectors b, int bOffset) {
        ...
        for (int w = 0; w < wordCount; w++) {
            long word1 = binaryVectorA[w] & WORD_MASK;
            long word2 = binaryVectorB[w] & WORD_MASK;
