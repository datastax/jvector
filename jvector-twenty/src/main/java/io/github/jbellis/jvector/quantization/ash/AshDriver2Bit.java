...
public class AshDriver2Bit implements AshDriver {
    ...
    private static final int BYTES_PER_WORD = Byte.BYTES;
    ...
    private static final long PIECE_MASK = 0b11;
    private static final int WORD_MASK = ~(~0 << BITS_PER_WORD);

    private final int dimension;

    public AshDriver2Bit(int dimension) {
    ...
    public PackedVectors create(int n) {
        ...
        return new PanamaPackedVectors(new byte[n][wordsPerVector]);
    }
    ...
    public void packInts(int[] toPack, int toPackOffset, PackedVectors out, int pvOffset) {
        ...
        byte[] binaryVector = ppv.vectors[pvOffset];
    ...
    public void unpackInts(PackedVectors packed, int pvOffset, int[] out, int outOffset) {
        ...
        final byte[] binaryVector = ppv.vectors[pvOffset];
        final int words = wordsForDims(dimension);
        if (words != binaryVector.length) {
            throw new IllegalArgumentException("binaryVector length mismatch");
        }
        ...
        for (int stack = 0; stack < stacks; stack++) {
            ...
            for (int lvl = 0; lvl < dimsPerWord; lvl++) {
                ...
                for (int wo = 0; wo < STACK_WIDTH; wo++) {
                    ...
                    int w = stack * STACK_WIDTH + wo;
                    int word = binaryVector[w] & WORD_MASK;
                    int piece = (word >>> (lvl * BIT_DEPTH)) & (int) PIECE_MASK;
    ...
    public float asymmetricScorePackedInts(PackedVectors packed, int pvOffset, float[] query, int qOffset) {
        ...
        for (; stack < stacks; stack++) {
            ...
            var fullPiece = ByteVector.fromArray(BYTE_SPECIES, binaryVector, stack * STACK_WIDTH);
            ...
            var piece0 = masked0.convertShape(VectorOperators.B2F, FLOAT_SPECIES, 0);
            var piece1 = masked1.convertShape(VectorOperators.B2F, FLOAT_SPECIES, 0);
            var piece2 = masked2.convertShape(VectorOperators.B2F, FLOAT_SPECIES, 0);
            var piece3 = masked3.convertShape(VectorOperators.B2F, FLOAT_SPECIES, 0);
    ...
    public float symmetricScorePackedInts(PackedVectors a, int aOffset, PackedVectors b, int bOffset) {
        ...
        final byte[] binaryVectorA = ppvA.vectors[aOffset];
        final byte[] binaryVectorB = ppvB.vectors[bOffset];
        ...
        for (int w = 0; w < wordCount; w++) {
            int word1 = binaryVectorA[w] & WORD_MASK;
            int word2 = binaryVectorB[w] & WORD_MASK;
    ...
    /**
     * Simple holder class for packed vectors data using byte arrays
     */
    private static class PanamaPackedVectors implements PackedVectors {
        byte[][] vectors;

        PanamaPackedVectors(byte[][] vectors) {
