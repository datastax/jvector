...
/**
 A MemDataSet is just a basic dataset.
 */
public class BaseDataSet implements DataSet, DataSet.CanScrub {
    ...
    public BaseDataSet(String name,
    ...
    protected BaseDataSet(String name,
                      VectorSimilarityFunction similarityFunction,
                      List<VectorFloat<?>> baseVectors,
                      List<VectorFloat<?>> queryVectors,
                      List<? extends List<Integer>> groundTruth,
...
//        List<List<Integer>> groundTruth
        ...
        Map<Integer, Integer> rawToScrubbed = new HashMap<>();
        {
            ...
            for (int i = 0; i < baseVectors.size(); i++) {
                ...
                var valid =
                    (similarityFunction == VectorSimilarityFunction.EUCLIDEAN) || Math.abs(normOf(v)) > 1e-5;
