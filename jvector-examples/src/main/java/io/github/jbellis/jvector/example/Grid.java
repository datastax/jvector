...
public class Grid {
                        ...
                        for (float neighborOverflow : neighborOverflowGrid) {
                            for (int efC : efConstructionGrid) {
                                for (var bc : buildCompressors) {
                                    ...
                                    runOneGraph(cache, featureSets, M, efC, neighborOverflow, addHierarchy, refineFinalGraph, compressor, numSplits, compressionGrid, topKGrid, usePruningGrid, benchmarks, ds, workDir);
                                }
                            }
                        }
                    }
    ...
    {
        runAll(ds, enableIndexCache, mGrid, efConstructionGrid, neighborOverflowGrid, addHierarchyGrid, refineFinalGraphGrid, featureSets, buildCompressors, numSplits, compressionGrid, topKGrid, usePruningGrid, null);
    }
                            ...
                            Path workDirectory) throws IOException
    {
        ...
        if(numSplits > 1) {
            ...
            compactIndex = buildCompaction(M, efConstruction, neighborOverflow, addHierarchy, refineFinalGraph, ds, workDirectory, numSplits);
        }
