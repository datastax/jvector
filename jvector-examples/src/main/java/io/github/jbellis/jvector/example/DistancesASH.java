...
public class DistancesASH {
    public static void testASHEncodings(String filenameBase, String filenameQueries, VectorSimilarityFunction vsf) throws IOException {
        ...
        int encodedDim = dimension; // change later
        int nQueries = 100;
        int nVectors = 10_000;
        ...
        // Generate a BQ for random vectors
        var ravv = new ListRandomAccessVectorValues(vectors, dimension);
        var ash = AsymmetricHashing.initialize(ravv, AsymmetricHashing.RANDOM, encodedDim);

        // Compress the vectors
        long startTime = System.nanoTime();
        var ashVecs = ash.encodeAll(ravv);
        ...
        for (int i = 0; i < nQueries; i++) {
            ...
            var f = ashVecs.scoreFunctionFor(q, vsf);
        ...
        System.out.println("\tASH Distance computations took " + duration + " seconds");
    ...
    public static void runSIFT() throws IOException {
        ...
        testASHEncodings(baseVectors, queryVectors, VectorSimilarityFunction.COSINE);
    }
