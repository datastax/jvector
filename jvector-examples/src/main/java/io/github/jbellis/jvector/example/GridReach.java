...
import io.github.jbellis.jvector.graph.disk.OnDiskReachGraphIndex;
import io.github.jbellis.jvector.graph.disk.OnDiskReachGraphIndexWriter;
...
public class GridReach {
            ...
            for (var cpSupplier : compressionGrid) {
                ...
                indexes.forEach((features, index) -> {
                    try (var cs = new ConfiguredSystem(ds, index, cv,
                                                       index instanceof OnDiskReachGraphIndex ? ((OnDiskReachGraphIndex) index).getFeatureSet() : Set.of())) {
            ...
            throws IOException
    {
        ...
        Map<Set<FeatureId>, OnDiskReachGraphIndexWriter> writers = new HashMap<>();
        Map<Set<FeatureId>, Map<FeatureId, IntFunction<Feature.State>>> suppliers = new HashMap<>();
        OnDiskReachGraphIndexWriter scoringWriter = null;
        int n = 0;
        ...
        for (var features : featureSets) {
            var graphPath = testDirectory.resolve("graph" + n++);
            var index = OnDiskReachGraphIndex.load(ReaderSupplierFactory.open(graphPath));
            ...
            throws FileNotFoundException
    {
        ...
        var builder = new OnDiskReachGraphIndexWriter.Builder(onHeapGraph, outPath)
    ...
    private static class BuilderWithSuppliers {
        public final OnDiskReachGraphIndexWriter.Builder builder;
        public final Map<FeatureId, IntFunction<Feature.State>> suppliers;

        public BuilderWithSuppliers(OnDiskReachGraphIndexWriter.Builder builder, Map<FeatureId, IntFunction<Feature.State>> suppliers) {
            ...
            if (cp.supportsCaching()) {
                try {
                    ...
                    try (var writer = new DataOutputStream(new BufferedOutputStream(Files.newOutputStream(path)))) {
                        compressor.write(writer, OnDiskReachGraphIndex.CURRENT_VERSION);
                    }
                } catch (IOException e) {
                    throw new UncheckedIOException(e);
                }
