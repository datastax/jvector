                        ...
                        for (float neighborOverflow : neighborOverflowGrid) {
                            for (int efC : efConstructionGrid) {
                                for (var bc : buildCompressors) {
                                    runOneGraph(shards, featureSets, M, efC, neighborOverflow, addHierarchy, refineFinalGraph, bc, compressionGrid, topKGrid, usePruningGrid, ds, testDirectory, benchmarks);
                                }
                            }
                        }
                    }
        ...
        for (int shard = 0; shard < shards; shard++) {
            ...
            if (buildCompressor == null) {
                ...
                indices = Grid.buildOnDisk(
                        ...
                        testDirectory, graphNamePrefices.get(shard), ds.name
                );
            }
            ...
            for (var cpSupplier : compressionGrid) {
                ...
                shardedIndices.forEach((features, indices) -> {
                    try (var cs = new MultiConfiguredSystem(ds, sharder, indices, cvs, sharder.getConverter())) {
                        Grid.testConfiguration(cs, topKGrid, usePruningGrid, M, efConstruction, neighborOverflow, addHierarchy, benchmarks);
                    } catch (Exception e) {
